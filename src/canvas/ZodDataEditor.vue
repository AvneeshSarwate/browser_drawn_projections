...
// Architecture note:
// ZodDataEditor is a schema-driven, local-draft editor. It always syncs its draft
// from modelValue, and the optional syncKey lets parents force a reset when the
// selection context changes (even if the value is identical), preventing cross-node
...
watch(
  [() => props.modelValue, () => props.schema, () => props.syncKey],
  ([val]) => {
