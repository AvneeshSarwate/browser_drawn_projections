  ...
  return RCE(
    ...
    spline !== undefined
      ? RCE(
          ...
          spline.type === 'mixed' ? RCE('option', { value: '' }, 'Mixed') : null,
...
export class MultiSegmentLineUtil extends ShapeUtil<MultiSegmentLineShape> {
  ...
  component(shape: MultiSegmentLineShape) {
    ...
    if (spline === 'linear') {
      ...
      const splinePointsString = closed ? catmullRomToBezierClosed(points) : catmullRomToBezier(points)
...
function catmullRomToBezierClosed(points) {
  if (!points || points.length < 2) return '';
